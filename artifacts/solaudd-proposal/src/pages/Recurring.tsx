...
export default function Recurring() {
  ...
  const handleExecute = async (plan: { id: string; recipientWallet: string; amountAudd: number; label: string; recipientName: string }) => {
    if (!connected) {
      toast({ title: "Connect a wallet to execute on-chain", variant: "destructive" });
      return;
    }
    ...
    try {
      ...
      await fetch(`/api/recurring/${plan.id}/execute`, {
        method: "POST",
      ...
      queryClient.invalidateQueries({ queryKey: getListRecurringPaymentsQueryKey() });
      toast({
        title: `A$${plan.amountAudd} sent to ${plan.recipientName}`,
        description: (
    ...
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          {!connected && (
            <p className="text-[10px] text-white/25 uppercase tracking-widest">Connect wallet to execute payments on-chain</p>
          )}
        </div>
