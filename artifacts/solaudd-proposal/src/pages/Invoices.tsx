...
export default function Invoices() {
  ...
  const form = useForm<z.infer<typeof invoiceSchema>>({
    ...
    defaultValues: { title: "", amountAudd: 0, recipientName: "", recipientWallet: "", recipientEmail: "", dueDate: "" },
  });
  ...
  const handlePayInvoice = async (invoice: { id: string; recipientWallet: string; amountAudd: number; title: string }) => {
    ...
    resetTx();
    try {
      const signature = await transfer(invoice.recipientWallet, invoice.amountAudd);
